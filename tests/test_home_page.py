...
class TestHome:
    ...
    def test_add_random_product_to_cart(self, driver, base_url):
        product = HomePage(driver, base_url).get_random_featured_product()
    ...
    def test_change_currency(self, driver, base_url, currency_code):
        home_page = HomePage(driver, base_url)

        prices = home_page.get_featured_products_prices()
        HeaderElement(driver).set_currency(currency_code)
        new_prices = home_page.get_featured_products_prices()
